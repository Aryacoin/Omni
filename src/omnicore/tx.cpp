...
int CMPTransaction::logicMath_SendToOwners()
{
    ...
    PrintToLog("\t    Transfer fee: %s %s\n", FormatDivisibleMP(transferFee), strMPProperty(OMNI_PROPERTY_AYA));

    // enough coins to pay the fee?
    if (OMNI_PROPERTY_AYA != property) {
        int64_t nBalanceFee = GetTokenBalance(sender, OMNI_PROPERTY_AYA, BALANCE);
    ...
    assert(update_tally_map(sender, OMNI_PROPERTY_AYA, -transferFee, BALANCE));

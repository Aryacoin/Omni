        ...
        throw runtime_error(
            RPCHelpMan{"omni_sendrawtx",
               "\nBroadcasts a raw Omni Layer transaction.\n",
               {
                   ...
                   {"referenceamount", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "a aryacoin amount that is sent to the receiver (minimal by default)\n"},
               },
               RPCResult{
                   "\"hash\"                  (string) the hex-encoded transaction hash\n"
               },
        ...
        throw runtime_error(
            RPCHelpMan{"omni_sendnonfungible",
               "\nCreate and broadcast a non-fungible send transaction.\n",
               {
                   ...
                   {"referenceamount", RPCArg::Type::STR, RPCArg::Optional::OMITTED_NAMED_ARG, "a aryacoin amount that is sent to the receiver (minimal by default)"},
               },
               RPCResult{
        ...
        throw runtime_error(
            RPCHelpMan{"omni_senddexsell",
               "\nPlace, update or cancel a sell offer on the distributed token/AYA exchange.\n",
               {
                   ...
                   {"amountdesired", RPCArg::Type::STR, RPCArg::Optional::NO, "the amount of aryacoins desired\n"},
        ...
        throw runtime_error(
            RPCHelpMan{"omni_sendupdatedexorder",
               "\nUpdates an existing sell offer on the distributed token/AYA exchange.\n",
               {
                   ...
                   {"amountdesired", RPCArg::Type::STR, RPCArg::Optional::NO, "the new amount of aryacoins desired\n"},
...
static UniValue omni_senddexpay(const JSONRPCRequest& request)
{
        ...
        throw runtime_error(
            RPCHelpMan{"omni_senddexpay",
               "\nCreate and broadcast payment for an accept offer.\n",
               {
                   ...
                   {"toaddress", RPCArg::Type::STR, RPCArg::Optional::NO, "the address of the seller\n"},
                   {"propertyid", RPCArg::Type::NUM, RPCArg::Optional::NO, "the identifier of the token to purchase\n"},
                   {"amount", RPCArg::Type::STR, RPCArg::Optional::NO, "the Aryacoin amount to send\n"},
               },
               RPCResult{
                   "\"hash\"                  (string) the hex-encoded transaction hash\n"
               },
    ...
    // Get accept offer and make sure buyer is not trying to overpay
    {
        ...
        if (nAmount > amountToPayInBTC) {
            throw JSONRPCError(RPC_MISC_ERROR, strprintf("Paying more than required: %lld AYA to pay for %lld tokens", FormatMoney(amountToPayInBTC), FormatMP(propertyId, amountAccepted)));
        }

        if (!isPropertyDivisible(propertyId) && nAmount < amountToPayInBTC) {
            throw JSONRPCError(RPC_MISC_ERROR, strprintf("Paying less than required: %lld AYA to pay for %lld tokens", FormatMoney(amountToPayInBTC), FormatMP(propertyId, amountAccepted)));
        }
    }
    ...
    if (request.fHelp || request.params.size() != 14)
        throw runtime_error(
            RPCHelpMan{"omni_sendissuancecrowdsale",
               ...
               RPCExamples{
                   HelpExampleCli("omni_sendissuancecrowdsale", "\"3JYd75REX3HXn1vAU83YuGfmiPXW7BpYXo\" 2 1 0 \"Companies\" \"Aryacoin Mining\" \"Quantum Miner\" \"\" \"\" 2 \"100\" 1483228800 30 2")
                   + HelpExampleRpc("omni_sendissuancecrowdsale", "\"3JYd75REX3HXn1vAU83YuGfmiPXW7BpYXo\", 2, 1, 0, \"Companies\", \"Aryacoin Mining\", \"Quantum Miner\", \"\", \"\", 2, \"100\", 1483228800, 30, 2")
               }
            }.ToString());
    ...
    if (request.fHelp || request.params.size() != 10)
        throw runtime_error(
            RPCHelpMan{"omni_sendissuancefixed",
               ...
               RPCExamples{
                   HelpExampleCli("omni_sendissuancefixed", "\"3Ck2kEGLJtZw9ENj2tameMCtS3HB7uRar3\" 2 1 0 \"Companies\" \"Aryacoin Mining\" \"Quantum Miner\" \"\" \"\" \"1000000\"")
                   + HelpExampleRpc("omni_sendissuancefixed", "\"3Ck2kEGLJtZw9ENj2tameMCtS3HB7uRar3\", 2, 1, 0, \"Companies\", \"Aryacoin Mining\", \"Quantum Miner\", \"\", \"\", \"1000000\"")
               }
            }.ToString());

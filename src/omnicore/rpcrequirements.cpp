...
{
    if ((0.01 * COIN) < amount) {
        throw JSONRPCError(RPC_TYPE_ERROR, "Reference amount higher is than 0.01 AYA");
    }
}
...
void RequireSaneDExFee(const std::string& address, uint32_t propertyId)
{
    ...
    if (poffer->getMinFee() > 2000000) {
        throw JSONRPCError(RPC_TYPE_ERROR, "Minimum accept fee is higher than 0.02 AYA (use override = true to continue)");
    }
}

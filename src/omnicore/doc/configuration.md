...
OmniAya can be configured by providing one or more optional command-line arguments:
```bash
$ omniayad -setting=value -setting=value
```

All settings can alternatively also be configured via the `aryacoin.conf`.

Depending on the operating system, the default locations for the configuration file are:

- Unix systems: `$HOME/.aryacoin/aryacoin.conf`
- Mac OS X: `$HOME/Library/Application Support/Aryacoin/aryacoin.conf`
- Microsoft Windows: `%APPDATA%/Aryacoin/aryacoin.conf`

A typical `aryacoin.conf` may include:
```
server=1
rpcuser=omnicorerpc
...
To run and use OmniAya, no explicit configuration is necessary.

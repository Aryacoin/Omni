...
OmniAya is a fork of Aryacoin, with the Omni Protocol feature support from OmniLayer for Bitcoin added as a new layer of functionality on top. As such interacting with the API is done in the same manner (JSON-RPC) as Aryacoin Core, simply with additional RPCs available for utilizing Omni Protocol features.

As all existing Aryacoin functionality is inherent to OmniAya, the RPC port by default remains as `9337` as per Aryacoin.  If you wish to run OmniAya in tandem with Aryacoin (eg. via a separate datadir) you may utilize the `-rpcport<port>` option to nominate an alternative port number.
...
| `referenceamount`   | string  | optional | a aryacoin amount that is sent to the receiver (minimal by default)                           |
...
Place, update or cancel a sell offer on the distributed token/AYA exchange.
...
| `amountdesired`     | string  | required | the amount of aryacoins desired                                                               |
...
Creates a new sell offer on the distributed token/AYA exchange.
...
Updates an existing sell offer on the distributed token/AYA exchange.
...
| `amountdesired`     | string  | required | the new amount of aryacoins desired                                                           |
...
Cancels existing sell offer on the distributed token/AYA exchange.
...
| `toaddress`         | string  | required | the address of the seller                                                                    |
| `propertyid`        | number  | required | the identifier of the token to purchase                                                      |
| `amount`            | string  | required | the Aryacoin amount to send                                                                   |
...
$ omnicore-cli "omni_sendissuancecrowdsale" \
    "3JYd75REX3HXn1vAU83YuGfmiPXW7BpYXo" 2 1 0 "Companies" "Aryacoin Mining" \
...
$ omnicore-cli "omni_sendissuancefixed" \
    "3Ck2kEGLJtZw9ENj2tameMCtS3HB7uRar3" 2 1 0 "Companies" "Aryacoin Mining" \
...
$ omnicore-cli "omni_sendissuancemanaged" \
    "3HsJvhr9qzgRe3ss97b1QHs38rmaLExLcH" 2 1 0 "Companies" "Aryacoin Mining" "Quantum Miner" "" ""
```
...
All aryacoins from the sender are consumed and if there are aryacoins missing, they are taken from the specified fee source. Change is sent to the fee source!
...
Result:
{
  ...
  "aryacoincoreversion" : "x.x.x",       // (string) Aryacoin Core version
...
```js
{
  ...
  "sendingaddress" : "address",    // (string) the Aryacoin address of the sender
  "referenceaddress" : "address",  // (string) a Aryacoin address used as reference (if any)
  "ismine" : true|false,           // (boolean) whether the transaction involes an address in the wallet
  "confirmations" : nnnnnnnnnn,    // (number) the number of transaction confirmations
  "fee" : "n.nnnnnnnn",            // (string) the transaction fee in aryacoins
...
[                                  // (array of JSON objects)
  {
    ...
    "seller" : "address",              // (string) the Aryacoin address of the seller
    "amountavailable" : "n.nnnnnnnn",  // (string) the number of tokens still listed for sale and currently available
    "aryacoindesired" : "n.nnnnnnnn",   // (string) the number of aryacoins desired in exchange
    "unitprice" : "n.nnnnnnnn" ,       // (string) the unit price (AYA/token)
    ...
    "accepts": [                       // (array of JSON objects) a list of pending "accept" orders
      {
        "buyer" : "address",               // (string) the Aryacoin address of the buyer
        ...
        "amounttopay" : "n.nnnnnnnn"       // (string) the amount in aryacoins needed finalize the trade
      },
      ...
    ]
  },
...
[                               // (array of JSON objects)
  {
    ...
    "issuer" : "address",           // (string) the Aryacoin address of the issuer on record
...
```js
{
  ...
  "recipients": [                // (array of JSON objects) a list of recipients
    {
      "address" : "address",         // (string) the Aryacoin address of the recipient
...
```bash
$ omnicore-cli "omni_createpayload_issuancefixed" 2 1 0 "Companies" "Aryacoin Mining" "Quantum Miner" "" "" "1000000"
```
...
```bash
$ omnicore-cli "omni_createpayload_issuancecrowdsale" 2 1 0 "Companies" "Aryacoin Mining" "Quantum Miner" "" "" 2 "100" 1483228800 30 2
```
...
```bash
$ omnicore-cli "omni_createpayload_issuancemanaged" 2 1 0 "Companies" "Aryacoin Mining" "Quantum Miner" "" ""
```

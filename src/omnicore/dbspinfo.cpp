...
uint32_t CMPSPInfo::peekNextSPID(uint8_t ecosystem) const
{
    ...
    switch (ecosystem) {
        case OMNI_PROPERTY_MSC: // Main ecosystem, MSC: 1, TMSC: 2, AYA: 3, First available SP = 4
            nextId = next_spid;
            break;
...
uint32_t CMPSPInfo::putSP(uint8_t ecosystem, const Entry& info)
{
    ...
    putSPGeneral(info, propertyId);

    return propertyId;
}

void CMPSPInfo::putSPGeneral(const Entry& info, const uint32_t& propertyId) {
...
bool CMPSPInfo::getSP(uint32_t propertyId, Entry& info) const
{
    ...
    if (OMNI_PROPERTY_MSC == propertyId) {
        info = implied_omni;
        return true;
    } else if (OMNI_PROPERTY_AYA == propertyId) {
        info = implied_aya;
        return true;
...
{
    // Special cases for constant SPs MSC and TMSC
    if (OMNI_PROPERTY_MSC == propertyId || OMNI_PROPERTY_TMSC == propertyId || OMNI_PROPERTY_AYA == propertyId) {
        return true;
    }
...
void CMPSPInfo::printAll() const
{
    ...
    for (uint32_t idx = OMNI_PROPERTY_MSC; idx <= OMNI_PROPERTY_AYA; idx++) {
        Entry info;

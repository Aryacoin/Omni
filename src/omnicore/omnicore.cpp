    ...
    if (0x80000000 & propertyId) {
        ...
        switch (propertyId) {
            ...
            case OMNI_PROPERTY_TMSC: str = "TOMN";
                break;
            case OMNI_PROPERTY_AYA: str = "AYA";
                break;
            case OMNI_PROPERTY_RAHA: str = "RAHA";
                break;
            default:
...
int mastercore_handler_block_begin(int nBlockPrev, CBlockIndex const * pBlockIndex)
{
    ...
    // Create property ID 3 AYA at one past OmniAya genesis block
    if (pBlockIndex->nHeight == ConsensusParams().GENESIS_BLOCK)
    {
        CMPSPInfo::Entry info;
        uint32_t propertyId = OMNI_PROPERTY_AYA;
...
/**
 * Returns the Aya address.
 *
 * @return The Exodus address
 */
const CTxDestination AyaAddress()
{
    if (TestNet())
    {
        static CTxDestination testAddress = DecodeDestination(aya_testnet);
        return testAddress;
    }
    else if (RegTest())
    {
        static CTxDestination testAddress = DecodeDestination(aya_regtest);
        return testAddress;
    }
    else
    {
        static CTxDestination mainAddress = DecodeDestination(aya_mainnet);
        return mainAddress;
    }
}

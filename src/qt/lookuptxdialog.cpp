    ...
    if (0<=populateResult) {
        ...
        switch(populateResult) {
            case MP_TX_NOT_FOUND:
                strText += "not a valid Aryacoin or Omni transaction.  Please check the transaction hash "
            ...
            case MP_TX_IS_NOT_OMNI_PROTOCOL:
                strText += "a Aryacoin transaction only.\n\nTip: You can use the debug console "
                           "'gettransaction' command to lookup specific Aryacoin transactions.";
            break;

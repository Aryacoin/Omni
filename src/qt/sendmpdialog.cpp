...
void SendMPDialog::updateFrom()
{
    ...
    if (currentSetFromAddress.empty()) {
        ...
        if (CheckFee(walletModel->wallet(), currentSetFromAddress, 16)) {
            ...
            ui->feeWarningLabel->setText("WARNING: The sending address is low on AYA for transaction fees. Please topup the AYA balance for the sending address to send Omni Layer transactions.");
